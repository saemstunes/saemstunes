...
const VARIANTS = {
  ...
  suggest_fix: [
    ...
    "Try `{bestMatch}` instead of `{path}` — that might work.",
    "Looks like you might be looking for `{bestMatch}` rather than `{path}`.",
    "Perhaps you meant `{bestMatch}`? `{path}` doesn't exist, but that does.",
    "Close! Try `{bestMatch}` — it's similar to `{path}` but actually works.",
    "We couldn't find `{path}`, but `{bestMatch}` might be what you're after.",
    "Almost there! `{bestMatch}` exists, while `{path}` doesn't.",
    "Did you intend to visit `{bestMatch}`? It's close to `{path}` but functional.",
    "Try navigating to `{bestMatch}` instead — `{path}` isn't available.",
    "`{path}` doesn't exist, but `{bestMatch}` might be what you need.",
    "We think you meant `{bestMatch}` rather than `{path}`.",
    "Close guess! `{bestMatch}` works, but `{path}` doesn't.",
    "Perhaps there's a typo? Try `{bestMatch}` instead of `{path}`.",
    "We couldn't locate `{path}`, but `{bestMatch}` is available and similar.",
    "`{bestMatch}` might be what you're looking for — `{path}` isn't found.",
    "Almost! `{bestMatch}` exists and might serve your needs better than `{path}`.",
    "Try `{bestMatch}` — it's the closest match to `{path}` that actually works.",
    "We suggest `{bestMatch}` as an alternative to `{path}`.",
    "`{path}` isn't available, but `{bestMatch}` might do the trick.",
    "Consider trying `{bestMatch}` instead of `{path}` — it's functional and similar.",
    "We couldn't find `{path}`, but `{bestMatch}` is right there waiting for you.",
    "Oops! Try `{bestMatch}` — it's like `{path}` but actually exists.",
    "Almost hit the right note! `{bestMatch}` works, while `{path}` doesn't.",
    "We think you're looking for `{bestMatch}` rather than `{path}`.",
    "Try the working alternative: `{bestMatch}` instead of `{path}`.",
    // MUSICAL SUGGEST FIX ADDITIONS
    "Did you mean to play `{bestMatch}`? We couldn't find the `{path}` track.",
    "Try tuning to `{bestMatch}` instead of `{path}` — that frequency actually broadcasts.",
    "Looks like you might be looking for the `{bestMatch}` song rather than `{path}`.",
    "Perhaps you meant to request `{bestMatch}`? The `{path}` record doesn't exist in our collection.",
    "Close harmony! Try `{bestMatch}` — it's similar to `{path}` but actually plays.",
    "We couldn't find `{path}` in our playlist, but `{bestMatch}` might be the melody you're after.",
    "Almost hit the right chord! `{bestMatch}` exists in our songbook, while `{path}` doesn't.",
    "Did you intend to listen to `{bestMatch}`? It's close to `{path}` but actually has sound.",
    "Try playing `{bestMatch}` instead — the `{path}` track isn't in our music library.",
    "The `{path}` song doesn't exist, but `{bestMatch}` might be the tune you need.",
    "We think you meant to queue up `{bestMatch}` rather than `{path}`.",
    "Close note! `{bestMatch}` plays beautifully, but `{path}` is silent.",
    "Perhaps there's a mix-up in your playlist? Try `{bestMatch}` instead of `{path}`.",
    "We couldn't locate `{path}` in our discography, but `{bestMatch}` is available and sounds similar.",
    "`{bestMatch}` might be the song you're humming — `{path}` isn't in our catalog.",
    "Almost found your rhythm! `{bestMatch}` exists and might groove better than `{path}`.",
    "Try `{bestMatch}` — it's the closest musical match to `{path}` that actually performs.",
    "We suggest `{bestMatch}` as a harmonious alternative to `{path}`.",
    "The `{path}` performance isn't available, but `{bestMatch}` might hit the right notes.",
    "Consider trying `{bestMatch}` instead of `{path}` — it's functional and sounds similar.",
    "We couldn't find `{path}` on our setlist, but `{bestMatch}` is ready for an encore.",
    "Oops! Try `{bestMatch}` — it's like `{path}` but actually makes music.",
    "Almost hit the right tempo! `{bestMatch}` works, while `{path}` doesn't have a beat.",
    "We think you're looking for the `{bestMatch}` composition rather than `{path}`.",
    "Try the working track: `{bestMatch}` instead of `{path}`.",
    "Did you mean to sample `{bestMatch}`? We couldn't find the `{path}` audio file.",
    "Try mixing `{bestMatch}` instead of `{path}` — that track actually exists in our studio.",
    "Looks like you're searching for `{bestMatch}` rather than `{path}` — check the liner notes.",
    "Perhaps you meant to stream `{bestMatch}`? The `{path}` single was never released.",
    "Close listening! Try `{bestMatch}` — it resonates, while `{path}` is just white noise.",
    "We couldn't find `{path}` on any album, but `{bestMatch}` might be the B-side you want.",
    "Almost found your jam! `{bestMatch}` is available for download, while `{path}` is not.",
    "Did you intend to bookmark `{bestMatch}`? It's close to `{path}` but actually streams.",
    "Try accessing `{bestMatch}` instead — the `{path}` concert was cancelled.",
    "The `{path}` remix doesn't exist, but `{bestMatch}` might be the version you remember.",
    "We think you meant to cue `{bestMatch}` rather than the phantom `{path}`.",
    "Close call! `{bestMatch}` is a real crowd-pleaser, but `{path}` never made it to vinyl.",
    "Perhaps there's static in your request? Try `{bestMatch}` instead of `{path}`.",
    "We couldn't locate `{path}` in any genre, but `{bestMatch}` is chart-ready and similar.",
    "`{bestMatch}` might be the earworm you're thinking of — `{path}` was never recorded.",
    "Almost nailed the solo! `{bestMatch}` performs live, while `{path}` is just a sound check.",
    "Try `{bestMatch}` — it's the closest acoustic match to `{path}` that actually amplifies.",
    "We suggest `{bestMatch}` as a melodic alternative to the missing `{path}`.",
    "The `{path}` tour isn't happening, but `{bestMatch}` might give you the same concert experience.",
    "Consider trying `{bestMatch}` instead of `{path}` — it's mastered and ready to play.",
    "We couldn't find `{path}` in our digital collection, but `{bestMatch}` is spinning right now.",
    "Oops! Try `{bestMatch}` — it's like `{path}` but actually made it past the demo stage.",
    "Almost found the right frequency! `{bestMatch}` broadcasts clearly, while `{path}` is just static.",
    "We think you're looking for the `{bestMatch}` session rather than `{path}`.",
    "Try the released version: `{bestMatch}` instead of the unreleased `{path}`.",
    "Did you mean to request `{bestMatch}`? We couldn't find `{path}` in our rotation.",
    "Try drumming up `{bestMatch}` instead of `{path}` — that beat actually drops.",
    "Looks like you might be humming `{bestMatch}` rather than `{path}`.",
    "Perhaps you meant to groove to `{bestMatch}`? The `{path}` rhythm section never showed up.",
    "Close harmony! Try `{bestMatch}` — it's in tune, while `{path}` is flat.",
    "We couldn't find `{path}` in our symphony, but `{bestMatch}` might be the movement you're after.",
    "Almost hit the bridge! `{bestMatch}` connects, while `{path}` leads nowhere.",
    "Did you intend to loop `{bestMatch}`? It's close to `{path}` but actually repeats.",
    "Try vibing to `{bestMatch}` instead — the `{path}` mood never set in.",
    "The `{path}` acoustic version doesn't exist, but `{bestMatch}` might be the unplugged sound you want.",
    "We think you meant to shuffle to `{bestMatch}` rather than `{path}`.",
    "Close remix! `{bestMatch}` drops the bass, but `{path}` never had a sound.",
    "Perhaps there's feedback in your search? Try `{bestMatch}` instead of `{path}`.",
    "We couldn't locate `{path}` in our sample library, but `{bestMatch}` is cleared and ready.",
    "`{bestMatch}` might be the hook you're thinking of — `{path}` never caught on.",
    "Almost found your anthem! `{bestMatch}` rallies the crowd, while `{path}` is just silence.",
    "Try `{bestMatch}` — it's the closest vocal match to `{path}` that actually sings.",
    "We suggest `{bestMatch}` as a rhythmic alternative to `{path}`.",
    "The `{path}` album never dropped, but `{bestMatch}` might give you the same vibes.",
    "Consider trying `{bestMatch}` instead of `{path}` — it's produced and ready to stream.",
    "We couldn't find `{path}` on any label, but `{bestMatch}` is signed and available.",
    "Oops! Try `{bestMatch}` — it's like `{path}` but actually made it to the chorus.",
    "Almost found the right pitch! `{bestMatch}` harmonizes perfectly, while `{path}` is off-key.",
    "We think you're looking for the `{bestMatch}` instrumental rather than `{path}`.",
    "Try the mastered track: `{bestMatch}` instead of the rough cut `{path}`."
  ]
};
